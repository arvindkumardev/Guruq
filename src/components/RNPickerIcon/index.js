...
export default class RNPickerIcon extends PureComponent {
  static propTypes = {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    const items = RNPickerIcon.handlePlaceholder({
    ...
    const { selectedItem, idx } = RNPickerIcon.getSelectedItem({
      items,
  ...
  constructor(props) {
    ...
    const { selectedItem } = RNPickerIcon.getSelectedItem({
      items,
      key: props.itemKey,
      value: props.value,
    });
    ...
    return items.map((item) => {
      return (
        <Picker.Item
          ...
          value={item.value}
          key={item.key}
          color={item.isPlaceholder ? Colors.black : Colors.dropdownColor}
        />
      );
    });
  }
    ...
    return (
      <View style={[defaultStyles.viewContainer, style.viewContainer]}>
        <TouchableOpacity
          ...
          <IconButtonWrapper iconImage={Images.serviceFilter} iconWidth={RfW(28)} iconHeight={RfH(28)} />
        </TouchableOpacity>
        <Modal
          testID="ios_modal"
          visible={showPicker}
  ...
  renderAndroidHeadless() {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={onOpen}
        activeOpacity={1}
        style={[style.inputAndroidContainer, { flex: 1 }]}>
        <View>
          <IconButtonWrapper iconImage={Images.serviceFilter} iconWidth={RfW(28)} iconHeight={RfH(28)} />
          <Picker
            style={[
