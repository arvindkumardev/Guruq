...
const discountLabels = [{ discount: '10%' }, { discount: '15%' }, { discount: '20%' }];
const studentGroupLabels = [{ studentNumber: '2-5' }, { studentNumber: '6-10' }, { studentNumber: '11-25' }];
const ClassPriceMatrix = (props) => {
  ...
  const [isDemoClass] = useState(offering.demoClass);
  ...
  const onValueChange = (value, index) => {
    if (isOnline) {
      setPriceMatrix((priceMatrix) => ({
        ...
        online: {
          ...priceMatrix.online,
          [index]: value ? parseInt(value, 10) : 0,
        },
      }));
    } else {
      setPriceMatrix((priceMatrix) => ({
  ...
  const renderItem = (item) => {
    return (
      <View style={[commonStyles.horizontalChildrenEqualSpaceView]}>
        ...
        <View style={styles.classInputFieldContainer}>
          <Input
            value={getItemValue(item)}
            onChangeText={(text) => onValueChange(text, `${item.value}`)}
            style={styles.classInputField}
            placeholder="Price"
  ...
  const renderNoOfStudents = (item) => {
    return (
      <View style={styles.noOfStudentContainer}>
        <Text style={styles.noOfStudentLabel}>{item.studentNumber}</Text>
      </View>
    );
  };
    ...
    <View style={styles.mainContainer}>
      ...
      <View style={{ marginTop: 36, paddingHorizontal: 16 }}>
        ...
        <FlatList
          ...
          scrollEnabled={false}
          ListFooterComponent={() => <View style={styles.flatListFooterView} />}
        />
      </View>
      <View style={styles.groupClassMainContainer}>
        <Text style={styles.subheadingText}>Group Class</Text>
        <View
          style={[
          ...
          <View style={styles.groupLabelMainContainer}>
            <Text style={styles.groupLabelText}>No. Of Students</Text>
          </View>
          <View style={styles.labelFlatListContainers}>
            <FlatList
              showsHorizontalScrollIndicator={false}
              data={studentGroupLabels}
        ...
        <View style={[commonStyles.horizontalChildrenEqualSpaceView]}>
          <View style={styles.groupLabelMainContainer}>
            <Text style={styles.groupLabelText}>Discount %</Text>
          </View>
          <View style={styles.labelFlatListContainers}>
            <FlatList
              showsHorizontalScrollIndicator={false}
